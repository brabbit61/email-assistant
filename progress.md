...
### #43 — T2.3 Budget-breach + failure-alert pings in worker
**Status:** Closed

The three remaining S2.2 operational pings added to `telegram.py` as siblings of `notify_p1`: `notify_budget` (daily soft-cap breach), `notify_failure` (N=5 consecutive crashed runs), `notify_oauth_death` (permanent `gmail.AuthError`), `notify_recovery` (worker back to normal). Each reuses `notify_p1`'s exact `action_events` audit-before-write and never-raises isolation — one row per ping, new `action_type`s, `gmail_message_id` NULL, a short `detail` for `assistant audit`. `cmd_run` restructured into three guard points: OAuth caught around `get_credentials`, everything else around the run body (a crash writes a `phase='triage', status='failed'` terminal, then alerts), budget + recovery on clean completion.

**Decided (formats/thresholds already signed off in #38; these are the implementation-shape forks #43 left open):**
- **Structural spine.** OAuth is caught narrowly around `get_credentials` — which runs *before* any run_event — so an auth death writes no run_event and can't feed the failure counter (matches #38's note); a general run crash is caught around the body; budget + recovery run only on clean completion. All ping code lives in `telegram.py`; `cli.py` gained the guard `try`/`except` + call sites.
- **"Failed run" = a `phase='triage'` invocation with no `ok`/`error` terminal.** On a crash `cmd_run` now writes a `status='failed'` triage terminal (rather than leaving the started row dangling): the streak = trailing `failed` rows since the last success, the dedupe/counting query is trivial, and `assistant status` correctly shows `Last run: failed` during an outage. Per-message errors (`status='error'`) still count as *completed*, not failed — the run finished and the checkpoint advanced.
- **Budget day = UTC, deviating from #38's "local calendar day."** The cost ledger stores UTC and `assistant costs` already groups by UTC day; there is no timezone in config. UTC keeps the breach ping and the `costs` breakdown agreeing about "today"; adding a tz just for a non-pausing nudge wasn't worth it. Dedupe = one confirmed `budget_ping` per UTC date.
- **One recovery ping covers both outage kinds.** Fires on the next clean completion when the latest `failure_ping` *or* `oauth_ping` is newer than the latest `recovery_ping` — so a resolved failure streak and a resolved auth death both get the single `✅ Worker recovered` message (#38 only drew the failure-recovery mockup). Dedupe is automatic (the recovery ping becomes the newest).
- **Operational pings ignore `dry_run`** (unlike P1, which stays suppressed). They report real worker state — real spend, real crashes, real dead auth — which exists in dry-run just as in live mode, and the dry-run trial is exactly when a crash-loop or runaway bill should surface. A hand-run `--dry-run` crash would count toward the streak, but it takes 5 consecutive to fire.
- **Shared `store.age`/`store.clock`** factored out of `cli.py` (was `cli._age`) so the pings and `assistant status` format timestamps the same way.

**How it was verified:** automated — ~13 new cases in `tests/test_telegram.py` (failure: fires only at threshold, dedupes within a streak, resets after a success, send-failure isolated; budget: fires once over cap same day, no-op under cap; oauth: fires then silent until re-auth, writes no run_events; recovery: fires after an unacked alert then dedupes, no-op without an alert, covers an oauth outage) + 4 wiring cases in `tests/test_cli.py` (oauth death pings and propagates with no run_event; five crashes ping once; recovery pings after the outage clears; budget pings when the daily cap is crossed). Live — Jenit confirmed all three AC checks 2026-07-14: lowering `daily_usd_soft_cap` produced one breach ping, five forced consecutive crashes produced the failure ping plus a recovery ping on the next good run, and a revoked token produced the OAuth-death ping.
