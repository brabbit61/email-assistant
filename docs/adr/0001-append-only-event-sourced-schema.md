...
**Status:** accepted (2026-07-13, T1.2 / issue #8)
...
- Cost/token records live in a separate `llm_calls` ledger (not columns on `classifications`) so agent digest/chat/draft calls — which have no classification — are still captured. This is a deliberate divergence from the ticket's literal wording.
